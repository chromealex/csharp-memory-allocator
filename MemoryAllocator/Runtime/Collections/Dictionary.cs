namespace ME.MA.Collections {
    ...
    public struct Dictionary<TKey, TValue> : IIsCreated where TKey : unmanaged where TValue : unmanaged {
        ...
        private int Initialize(ref MemoryAllocator allocator, int capacity) {
            var prime = System.Collections.Generic.HashHelpers.GetPrime(capacity);
            this.freeList = -1;
        ...
        private void Resize(ref MemoryAllocator allocator) {
			E.IS_CREATED(this);
            this.Resize(ref allocator, System.Collections.Generic.HashHelpers.ExpandPrime(this.count));
        }
